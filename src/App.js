            ...
            <Layout style={{ minHeight: '100vh' }}>
              ...
              >
                ...
                <Menu selectkeys={['3']} theme="dark" defaultSelectedKeys={[sessionStorage.getItem('name')]} mode="inline" onOpenChange={this.onOpenChange.bind(this)} openKeys={this.state.openKeys}>
                  ...
                  >
                    <Menu.Item key="8"><Link onClick={this._click.bind(this, 8, 'sub3')} to='/mediaManagement'>介质管理</Link></Menu.Item>
                  </SubMenu>
                  <SubMenu
                    key="sub4"
                    title={<span><Icon type="alert" /><span>告警处理</span></span>}
                  >
                    <Menu.Item key="9"><Link onClick={this._click.bind(this, 9, 'sub4')} to='/setWarning'>告警设置</Link></Menu.Item>
                    <Menu.Item key="10"><Link onClick={this._click.bind(this, 10, 'sub4')} to='/recordWarning'>告警记录</Link></Menu.Item>
                    <Menu.Item key="11"><Link onClick={this._click.bind(this, 11, 'sub4')} to='/informWarning'>告警通知组</Link></Menu.Item>
                  </SubMenu>
                  <SubMenu
                    key="sub5"
                    title={<span><Icon type="cluster" /><span>问题分析</span></span>}
                  >
                    <Menu.Item key="12"><Link onClick={this._click.bind(this, 12, 'sub5')} to='/analyze'>链路分析</Link></Menu.Item>
                    <Menu.Item key="13"><Link onClick={this._click.bind(this, 13, 'sub5')} to='/monito'>资源监控</Link></Menu.Item>
                  </SubMenu>
                  <SubMenu
                    key="sub6"
                    title={<span><Icon type="crown" /><span>智能预测</span></span>}
                  >
                    <Menu.Item key="14"><Link onClick={this._click.bind(this, 14, 'sub6')} to='/systemPrediction'>系统资源预测</Link></Menu.Item>
                    <Menu.Item key="15"><Link onClick={this._click.bind(this, 15, 'sub6')} to='/callLinkPrediction'>调用链路预测</Link></Menu.Item>
                    <Menu.Item key="16"><Link onClick={this._click.bind(this, 16, 'sub6')} to='/requestTime'>请求耗时预测</Link></Menu.Item>
                    <Menu.Item key="17"><Link onClick={this._click.bind(this, 17, 'sub6')} to='/baselinePrediction'>基线预测</Link></Menu.Item>
                  </SubMenu>
                  <SubMenu
                    key="sub7"
                    title={<span><Icon type="setting" /><span>配置</span></span>}
                  >
                    <Menu.Item key="18"><Link onClick={this._click.bind(this, 18, 'sub7')} to='/configure'>grok规则</Link></Menu.Item>
                    <Menu.Item key="19"><Link onClick={this._click.bind(this, 19, 'sub7')} to='/abnormalConfig'>异常配置</Link></Menu.Item>
                  </SubMenu>
                </Menu>
              </Sider>
              <Layout>
